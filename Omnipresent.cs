...
namespace Mazeinator
{
    ...
    internal class Omnipresent : INotifyPropertyChanged
    {
        ...
        private bool _isSquare = true; public bool IsSquare { get { return _isSquare; } set { _isSquare = value; OnPropertyChanged(nameof(IsSquare)); } }

        private int _nodeCount = 0; public int NodeCount { get { return _nodeCount; } set { _nodeCount = value; OnPropertyChanged(nameof(NodeCount)); } }
        private long _lastGenTime = 0; public long LastGenTime { get { return _lastGenTime; } set { _lastGenTime = value; OnPropertyChanged(nameof(LastGenTime)); } }
        private long _lastRenderTime = 0; public long LastRenderTime { get { return _lastRenderTime; } set { _lastRenderTime = value; OnPropertyChanged(nameof(LastRenderTime)); } }
        private long _renderSizeX = 0; public long RenderSizeX { get { return _renderSizeX; } set { _renderSizeX = value; OnPropertyChanged(nameof(RenderSizeX)); } }
        private long _renderSizeY = 0; public long RenderSizeY { get { return _renderSizeY; } set { _renderSizeY = value; OnPropertyChanged(nameof(RenderSizeY)); } }
        private long _canvasSizeX = 0; public long CanvasSizeX { get { return _canvasSizeX; } set { _canvasSizeX = value; OnPropertyChanged(nameof(CanvasSizeX)); } }
        private long _canvasSizeY = 0; public long CanvasSizeY { get { return _canvasSizeY; } set { _canvasSizeY = value; OnPropertyChanged(nameof(CanvasSizeY)); } }
        private int _percent = 20; public int Percent { get { return _percent; } set { _percent = value; OnPropertyChanged(nameof(Percent)); } }

        private string _status = "Ready"; public string Status { get { return _status; } set { _status = value; OnPropertyChanged(nameof(Status)); } }
        private string _currentFilePath = null;
        private BitmapImage _maze = null; public BitmapImage Maze { get { return _maze; } set { _maze = value; OnPropertyChanged(nameof(Maze)); } }
        ...
        public void DisplayMaze(Tuple<int, int> CanvasSize)
        {
            Stopwatch RenderTime = new Stopwatch();
            RenderTime.Start();

            Maze = Utilities.BitmapToImageSource(MainMaze.DisplayMaze(CanvasSize.Item1, CanvasSize.Item2, IsSquare));
        ...
        public void Export(Tuple<int, int> CanvasSize)
        {
            ...
            try
            {
                if (dialog.ShowDialog() == true)
                {
                    ...
                    MainMaze.DisplayMaze(CanvasSize.Item1 * 2, CanvasSize.Item2 * 2, false).Save(dialog.FileName);
                    Status = "Export done";
                }
            }
            catch (Exception exc)
